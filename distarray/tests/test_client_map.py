...
class TestClientMap(unittest.TestCase):
    ...
    def test_2D_bn(self):
        nrows, ncols = 31, 53
        cm = client_map.Distribution(self.ctx, (nrows, ncols), {0:'b'}, (4,1))
        chunksize = (nrows // 4) + 1
        for _ in range(100):
    ...
    def test_2D_bb(self):
        nrows, ncols = 3, 5
        nprocs_per_dim = 2
        cm = client_map.Distribution(self.ctx, (nrows, ncols), ('b', 'b'),
    ...
    def test_2D_cc(self):
        nrows, ncols = 3, 5
        nprocs_per_dim = 2
        cm = client_map.Distribution(self.ctx, (nrows, ncols), ('c', 'c'),
                (nprocs_per_dim, nprocs_per_dim))
        for r in range(nrows):
            for c in range(ncols):
